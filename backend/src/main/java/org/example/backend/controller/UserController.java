...
public class UserController {
    ...
    public Result<org.example.backend.entity.User> register(@RequestBody RegisterRequest request) {
        if (verificationCodes.containsKey(request.getVerificationCode()) && verificationCodes.get(request.getVerificationCode()).equals(request.getEmail())) {
            // 验证通过，可以注册，这里可以将userInfo保存到数据库中
            verificationCodes.remove(request.getVerificationCode()); // 移除已验证的验证码
        } else {
